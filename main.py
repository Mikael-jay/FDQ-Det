...
def main(args):
    ...
    if args.resume:
        if os.path.isdir(args.resume):
            raise ValueError(f"args.resume 指向的是目录而非文件，请检查路径：{args.resume}")
    ...
    if (not args.resume) and args.pretrain_model_path:
        ...
        # 自适应处理 CCM 通道不匹配（例如旧 checkpoint 中 conv1 是 512->256，新模型是 256->256）
        ...
        for k, v in _tmp_st.items():
            if k in model_state:
                if v.shape == model_state[k].shape:
                    ...
                    # 尝试按维度切片（取前部分）
                    if all(s >= t for s, t in zip(v.shape, model_state[k].shape)):
                        slices = tuple(slice(0, t) for t in model_state[k].shape)
                        adapted_st[k] = v[slices]
                        logger.info(f"[ADAPT] {k}: {tuple(v.shape)} -> {tuple(model_state[k].shape)} (sliced)")
                    else:
                        # 无法适配，跳过加载
                        logger.warning(f"[SKIP] {k}: incompatible shape {tuple(v.shape)} vs {tuple(model_state[k].shape)}")
            else:
                adapted_st[k] = v

        _load_output = model_without_ddp.load_state_dict(adapted_st, strict=False)
    ...
    if args.eval:
        ...
        test_stats, coco_evaluator = evaluate(
            ...
            device, args.output_dir, wo_class_error=wo_class_error, args=args
        )
        if args.output_dir:
