...
class DensityMapLossWithRanking(nn.Module):
    def __init__(
        ...
        **kwargs,
        ...
        # expected integral (unit mass) per single object - used to normalize densities into counts
        ...
        # legacy/optional args (keep to avoid unexpected-kw errors)
        ...
        # feature-level ranking weight (encourage feature energy to align with GT density)
        self.weight_feature_ranking = kwargs.get('weight_feature_ranking', 0.1)

    def forward(self, pred_density, gt_density, ccm_feature=None, **kwargs):
        eps = 1e-6

        # Allow caller to provide per-call single_object_integral (engine will pass this)
        single_int = kwargs.get('single_object_integral', None)
        ...
        # Normalize densities into per-pixel counts (objects per pixel) to avoid large-magnitude gradients
        pred_density = pred_density / (single_int + eps)
        gt_density = gt_density / (single_int + eps)

        # =============================
        # 1. Support Mask (GT-defined)
        # =============================
        # compute per-sample max over spatial dims robustly (flatten H,W)
        gt_flat = gt_density.flatten(2)  # [B, C, H*W]
        gt_max = gt_flat.max(dim=2, keepdim=True)[0].unsqueeze(-1)  # [B, C, 1, 1]
        support_mask = gt_density > (0.05 * gt_max)
        ...
        # 4. Support Loss (HARD)
        # =============================
        support_loss = torch.mean(pred_density * (~support_mask).float())
        ...
        # =============================
        # Feature-level Ranking Loss (align feature energy with GT density)
        # =============================
        feature_ranking_loss = torch.tensor(0.0, device=pred_density.device)
        feature_ranking_info = {}
        if ccm_feature is not None:
            # ccm_feature: [B, C, H, W] -> feature energy proxy [B,1,H,W]
            feature_score = torch.norm(ccm_feature, p=2, dim=1, keepdim=True)
            feature_ranking_loss, feature_ranking_dict = self.ranking_loss_fn(
                feature_score, gt_density
            )
            # prefix keys to avoid collision
            feature_ranking_info = {f'feature_{k}': v for k, v in feature_ranking_dict.items()}
        ...
        total_loss = (
            ...
            self.weight_feature_ranking * feature_ranking_loss
        )
        ...
        loss_dict = {
            ...
            'ranking_loss': ranking_loss.item(),
            'feature_ranking_loss': feature_ranking_loss.item() if isinstance(feature_ranking_loss, torch.Tensor) else float(feature_ranking_loss),
            'total_loss': total_loss.item(),
        }
        loss_dict.update(ranking_dict)
        loss_dict.update(feature_ranking_info)
